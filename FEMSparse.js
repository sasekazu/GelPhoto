...
function FEMSparse(initpos, tri) {
...
FEMSparse.prototype.makeMatrixKe = function (young, poisson, density, thickness) {
	// Bマトリクスを作成
...
FEMSparse.prototype.makeMatrixK = function () {
...
// クリック時の処理
FEMSparse.prototype.selectHoldNodes = function (mousePos) {
...
// 境界条件の設定
FEMSparse.prototype.setBoundary = function (clickState, mousePos, gravityFlag) {
...
FEMSparse.prototype.makeMatrixKSW = function () {
	var TriNum = this.tri.length;
	this.K = numeric.rep([2 * this.pos.length, 2 * this.pos.length], 0);
...
FEMSparse.prototype.calcDeformation = function () {
...
// 境界条件を設定して変形計算を行う
// 境界条件は y=0 を固定，ノード番号spNodeに強制変位disp[2]を与える
FEMSparse.prototype.calcDynamicDeformation = function (dt) {

	// Stiffness Warping法の場合、剛性マトリクスを修正する
	var timeRot0 = new Date();
	this.makeMatrixKSW();
	var timeRot1 = new Date();
	console.log(" rot: " + (timeRot1 - timeRot0) + " [ms]");

	var timePrep0 = new Date();
	...
	// Kffの作成（疎行列）
	var KffRowIdx = numeric.linspace(0, 0, 4*f*f);
	var KffColIdx = numeric.linspace(0, 0, 4*f*f);
	var KffValues = numeric.linspace(0, 0, 4*f*f);
	for(var i = 0; i < f; i++) {
		for(var j = 0; j < f; j++) {
			for(var k = 0; k < 2; k++) {
				for(var l = 0; l < 2; l++) {
					KffRowIdx[(4*f)*i+4*j+2*k+l] = 2*i+k;
					KffColIdx[(4*f)*i+4*j+2*k+l] = 2*j+l;
					KffValues[(4*f)*i+4*j+2*k+l] = this.K[2 * this.flist[i] + k][2 * this.flist[j] + l];
				}
			}
		}
	}
	var Kff = numeric.cooToCcs([KffRowIdx, KffColIdx, KffValues]);

	// Kfdの作成（疎行列）
	/*
	var KfdRowIdx = numeric.linspace(0, 0, 4*f*d);
	var KfdColIdx = numeric.linspace(0, 0, 4*f*d);
	var KfdValues = numeric.linspace(0, 0, 4*f*d);
	for(var i = 0; i < f; i++){
		for(var j = 0; j < d; j++){
			for(var k = 0; k < 2; k++){
				for(var l = 0; l < 2; l++){
					KfdRowIdx[(4*d)*i+4*j+2*k+l] = 2*i+k;
					KfdColIdx[(4*d)*i+4*j+2*k+l] = 2*j+l;
					KfdValues[(4*d)*i+4*j+2*k+l] = this.K[2 * this.flist[i] + k][2 * this.dlist[j] + l];
				}
			}
		}
	}
	var Kfd = numeric.cooToCcs([KfdRowIdx, KfdColIdx, KfdValues]);
	*/
	...
	// Mの作成（疎行列）, Mは対角行列なのでIdx配列は行・列で共通
	var MRowIdx = numeric.linspace(0, 0, 2*f);
	var MColIdx = numeric.linspace(0, 0, 2*f);
	var MValues = numeric.linspace(0, 0, 2*f);
	for(var i = 0; i < f; i++) {
		MRowIdx[2*i+0] = 2*i;
		MColIdx[2*i+0] = 2*i;
		MValues[2*i+0] = this.mass[this.flist[i]];
		MRowIdx[2*i+1] = 2*i+1;
		MColIdx[2*i+1] = 2*i+1;
		MValues[2*i+1] = this.mass[this.flist[i]];
	}
	var M = numeric.cooToCcs([MRowIdx, MColIdx, MValues]);
	...
	var timePrep1 = new Date();
	console.log(" prep: " + (timePrep1 - timePrep0) + " [ms]");


	var timeCalc0 = new Date();
	var Mleft1 = numeric.ccsmul(M, (1 + this.alpha));
	var Mleft2 = numeric.ccsmul(Kff, dt * (this.beta + dt));
	var Mleft = numeric.ccsadd(Mleft1, Mleft2);

	var Vright1 = numeric.ccsDotMatVec(M, vf);
	var Vright2 = numeric.ccsDotMatVec(Kff, xf);


	Vright2 = numeric.mul(Vright2,-1);
	var tmp = numeric.dot(Kfd, xd);
	//var tmp = numeric.ccsDotMatVec(Kfd, xd);
	Vright2 = numeric.sub(Vright2, tmp);
	Vright2 = numeric.sub(Vright2, fof);
	Vright2 = numeric.add(Vright2, this.ff);
	Vright2 = numeric.mul(Vright2, dt);
	var Vright = numeric.add(Vright1, Vright2);
	var timeCalc1 = new Date();
	console.log(" calc: " + (timeCalc1 - timeCalc0) + " [ms]");

	var timeSolve0 = new Date();
	var LUP = numeric.ccsLUP(Mleft);
	vf = numeric.ccsLUPSolve(LUP, Vright);
	var timeSolve1 = new Date();
	console.log(" solve: " + (timeSolve1-timeSolve0) + " [ms]");
...
FEMSparse.prototype.modifyPosCld = function (xmin, ymin, xmax, ymax) {
	for(var i = 0; i < this.pos.length; i++) {
...
FEMSparse.prototype.calcStress = function () {
	for(var i = 0; i < this.tri.length; i++) {
		var xe = [0, 0, 0, 0, 0, 0];
		for(var j = 0; j < 3; j++) {
...
FEMSparse.prototype.removeElement = function () {
	for(var i = 0; i < this.tri.length; i++) {
